...
class Graph_CapgMyo:
    ...
    def KNN1_SW(self):
        ...
        for i in range(row_sensors):
            if i==0: 
                edge['lst_{}'.format(i)]=[1,row_sensors,row_sensors+1,self.num_sensor-row_sensors,self.num_sensor-row_sensors+1]
            elif i==(row_sensors-1): 
                edge['lst_{}'.format(i)]=[row_sensors-2,(row_sensors*2)-2,(row_sensors*2)-1,self.num_sensor-2,self.num_sensor-1]
            else: 
                edge['lst_{}'.format(i)]=[i-1, i+1, i+(row_sensors-1), i+row_sensors, i+row_sensors+1, i+(self.num_sensor-row_sensors-1), 
                              i+(self.num_sensor-row_sensors), i+(self.num_sensor-row_sensors+1)]
        ...
        for _ in range(int(len(self.df)/self.num_sensor)-1):
            ...
            for k in range(self.num_sensor):  #SW 
    ...
    def KNN1_SW(self): 
        ...
        for sensors in range(self.num_sensor):
            for i in range(len(edge['lst_{}'.format(sensors)])):
                ...
                lst_tmp_2.append(edge['lst_{}'.format(sensors)][i])
        ...
        for i in range(int(len(self.df)/self.num_sensor)-1):
            for idx in range(len(lst_tmp_1)):
                lst_tmp_1[idx] = lst_tmp_1[idx] + self.num_sensor
                lst_tmp_2[idx] = lst_tmp_2[idx] + self.num_sensor

            lst1_final = lst1_final + lst_tmp_1
            lst2_final = lst2_final + lst_tmp_2
                
            for k in range(self.num_sensor):  #SW 부분
                lst2_final.append((i+1)*self.num_sensor + k)  #128, 129
                lst1_final.append(num_cnt)
                num_cnt = num_cnt + 1
        
        return self.generate_graph(lst1_final, lst2_final)
